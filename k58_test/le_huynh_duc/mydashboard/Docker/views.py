...
class IndexView(tables.DataTableView):
    ...
    template_name = 'mydashboard/Docker/index.html'
...
def handleRequest(request):
    ...
    for container in containers_id:
        if (containers[container]['aliases'][0] == request.GET['container_options']):
            series_container ={}
            series_container['name']= containers[container][0] +' : ' +  request.GET['resource_options']
            ...
            if resource_option == 'CPU':
                for stat in stats:
                    ...
                    data_content['x'] = stat['cpu']['timestamp'][:19]
                    data_resource.append(data_content)
            elif resource_option == 'RAM':
                for stat in stats:
                    data_content = {}
                    data_content['y'] = stat['memory']['usage']
                    data_content['x'] = stat['memory']['timestamp'][:19]
                    data_resource.append(data_content)
